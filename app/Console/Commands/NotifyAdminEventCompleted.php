            ...
            if (!$existingNotification) {
                ...
                foreach ($admins as $admin) {
                    Notification::create([
                        ...
                        'message' => "Event for booking #{$booking->id} ({$booking->event_type}) by {$booking->user->name} on {$booking->event_date->format('F d, Y')} has passed. You can now mark it as successful.",
                        'read' => false,
                        'data' => [
                ...
                $notificationCount++;
                $this->info("Sent notification for booking #{$booking->id} - {$booking->event_type} event on {$booking->event_date->format('F d, Y')}");
            }
        }
